...
import { verifyIdnormWebhookSignature } from "@/lib/idnorm";

export async function POST(request: NextRequest) {
  try {
    const rawBody = await request.text();
    const signature = request.headers.get("x-idnorm-signature");

    const isValid = await verifyIdnormWebhookSignature(rawBody, signature);
    if (!isValid) {
    ...
    console.error("IDNorm Webhook error:", error);
